...
const openrosa_xpath_extensions = function () {
  const TOO_MANY_ARGS = new Error('too many args'),
    ...
    format_date = function (date, format) {
      ...
      const locale = (globalThis.window as any)?.enketoFormLocale;
    ...
    ret: { func; process; XPR; _now } = {} as any;
  ...
  const func = {
    ...
    'decimal-time': function (r) {
      ...
      if (
        ...
        if (d.toString() === 'Invalid Date') {
          ...
          dec =
            (d.getSeconds() / 3600 + d.getMinutes() / 60 + d.getHours()) / 24;
        }
      } else {
        dec = NaN;
      }
    ...
    exp10: function (r) {
      return XPR.number(Math.pow(10, asNumber(r)));
    },
    false: function () {
    ...
    pow: function (x, y) {
      return XPR.number(Math.pow(asNumber(x), asNumber(y)));
    },
    random: function () {
...
export default openrosa_xpath_extensions;
