...
RETURNS TRIGGER AS $$
BEGIN
    -- Compare full row (deep JSONB included)
    IF to_jsonb(OLD.*) IS NOT DISTINCT FROM to_jsonb(NEW.*) THEN
        -- Cancel the update (row is unchanged)
        RETURN NULL;
    END IF;

    -- Otherwise, allow update to proceed
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
    ...
    LOOP
        ...
        ELSIF jsonb_typeof(old_val) = 'object'
           AND jsonb_typeof(new_val) = 'object' THEN
            SELECT a.added, a.removed
            INTO sub_added, sub_removed
            FROM audit.jsonb_recursive_diff(old_val, new_val) a;
...
BEGIN
    ...
    -- Build JSONB of primary key(s) (works for single-column PKs; can be extended for composite)
    ...
    IF (TG_OP = 'UPDATE') THEN
        ...
        -- Compute recursive diff (fixed ambiguity)
        ...
        INSERT INTO audit.audit_log(
            ...
            old_data, new_data, diff_added, diff_removed,
            changed_by, changed_by_email
        )
        VALUES (
            TG_TABLE_SCHEMA, TG_TABLE_NAME, TG_OP, pk,
            to_jsonb(OLD.*), to_jsonb(NEW.*), added, removed,
            auth.uid(), user_email
        );
    ...
    ELSIF (TG_OP = 'DELETE') THEN
        INSERT INTO audit.audit_log(
            schema_name, table_name, operation, row_pk, old_data,
            changed_by, changed_by_email
        )
        VALUES (
