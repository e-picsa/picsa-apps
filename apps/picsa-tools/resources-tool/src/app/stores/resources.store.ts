...
export class ResourcesStore {
  ...
  private sortResources<T extends IResourceItemBase>(resources: T[]) {
    return resources.sort((a: IResourceItemBase, b: IResourceItemBase) => (b.priority ?? -99) - (a.priority ?? -99));
  }
   ...
   */
  private populateCountryCollections(resourcesById: { [id: string]: IResource }) {
    ...
    const allCollections = Object.values(resourcesById)
      ...
      .map((r) => {
        ...
        collection.childResources = collection.childResources.filter((childKey) =>
          resourcesById.hasOwnProperty(childKey)
        );
  ...
  async openFileResource(resource: IResourceFile) {
    if (this.isNative) {
      try {
        this.storageService.openFile(`resources/${resource.filename}`, resource.mimetype);
      } catch (error) {
        console.error(error);
      }
    } else {
  ...
  private async checkHardcodedData() {
    ...
    const unCachedFiles = Object.values(contents).filter((entry) => !this.storageService.checkFileCached(entry));
