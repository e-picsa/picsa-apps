...
export class TranslationDashboardService extends PicsaAsyncService {
  ...
  public generateTranslationID(row: Partial<ITranslationInsert>) {
    if (row.id) {
      return row.id;
    }
  ...
  public exportJson(locale: string, shouldDownload = false) {
    ...
    // helper to get the translation key (id if custom/override or text empty, otherwise text)
    const getTranslationKey = (row: ITranslationRow) => {
      const { text, id } = row;
      const isCustomId = !text || id !== this.generateTranslationID(row);
      return isCustomId ? id : text;
    };

    const sortedData = data.sort((a, b) => {
      const aKey = getTranslationKey(a);
      const bKey = getTranslationKey(b);
      return aKey.toLowerCase() > bKey.toLowerCase() ? 1 : -1;
    });

    for (const row of sortedData) {
      const key = getTranslationKey(row);
      const existingTranslation = translations.get(key);
      ...
      if (translatedText !== null && translatedText !== undefined) {
        if (existingTranslation && translatedText !== existingTranslation) {
          console.warn('Duplicate translation skipped', { key, translatedText, existingTranslation });
        } else {
