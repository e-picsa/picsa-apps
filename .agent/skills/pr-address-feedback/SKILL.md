...
_Include the review freshness status in your summary table._
...
1. **Apply Accepted Fixes**:
   ...
   - Keep comments concise: explain _why_ something is done if non-obvious, never leave internal monologue or questions in code comments.
...
2. **Run Local Verification (Prevent CI Failure Loops)**:
   ...
   _(Or run targeted checks for the touched library/app, e.g. `yarn nx test <project-name>`)_.
...
1. **Present Local Changes & Ask for Push Approval (MANDATORY)**:
   ...
   - **Do NOT commit or push automatically** unless the user explicitly gave permission in their initial prompt (e.g., _"implement, commit, and push"_).
   - Ask the user for confirmation:
     > _"Changes verified locally. Would you like me to commit and push to remote (`origin/<branch>`)?"_
