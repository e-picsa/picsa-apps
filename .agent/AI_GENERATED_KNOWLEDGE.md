...
2. **Local Inbucket SMTP**: The local Supabase environment runs an email sink called Inbucket (formerly Mailpit). This service exposes an SMTP server on port `2500` inside the docker network.
3. **Usage via Edge Functions**: You can use `npm:nodemailer` in a Deno Edge function to route emails to `host: 'inbucket', port: 2500, ignoreTLS: true`. These emails will then appear in the local Supabase studio at `http://localhost:54324/`.
