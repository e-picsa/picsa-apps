...
async function processMapTiles() {
  const mapTilesDir = path.resolve('apps/picsa-tools/climate-tool/src/assets/mapTiles');
  const found = globSync('**/*.png', { cwd: mapTilesDir });
  const results: any[] = [];
  ...
  for (const filepath of found) {
    ...
    await sharp(input).webp({ nearLossless: true, alphaQuality: 0 }).toFile(output);
    const { size: sizeAfter } = statSync(output);
    unlinkSync(input);
    if (sizeBefore && sizeAfter) {
      const saving = Math.round((100 * (sizeBefore - sizeAfter)) / sizeBefore) + '%';
